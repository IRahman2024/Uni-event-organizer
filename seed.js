...
const rawSeedData = [
    ...
    // 💾 Set 2: A different event type with multiple fields
    {
        eventData: [{
    ...
    // 💾 Set 4: A smaller, music-focused event
    {
        eventData: [{
...
const seed = async () => {
    for (const rawData of rawSeedData) {
        try {
            ...
            console.log(`✅ Created event: ${event.eventTitle} (ID: ${event.id})`);
        } catch (error) {
            console.error(`❌ Error seeding event: ${rawData.eventData[0].eventTitle}`, error);
        }
    }
